...
#define PYRectangleTagMaxCol 4
#define PYRectangleTagMargin 10
...
- (void)setup
{
    ...
    UIButton *emptySearchHistoryLabel = [[UIButton alloc] init];
    [emptySearchHistoryLabel setTitleColor:[UIColor darkGrayColor] forState:UIControlStateNormal];
//    emptySearchHistoryLabel.textColor = [UIColor darkGrayColor];
    emptySearchHistoryLabel.titleLabel.font = [UIFont systemFontOfSize:13];
    emptySearchHistoryLabel.userInteractionEnabled = YES;
    [emptySearchHistoryLabel setTitle:[NSBundle py_localizedStringForKey:PYSearchEmptySearchHistoryText] forState:UIControlStateNormal];
    [emptySearchHistoryLabel setImage:[NSBundle py_imageNamed:@"emptySearch"] forState:UIControlStateNormal];
    
    [emptySearchHistoryLabel addTarget:self action:@selector(emptySearchHistoryDidClick) forControlEvents:UIControlEventTouchUpInside];
    emptySearchHistoryLabel.titleLabel.textAlignment = NSTextAlignmentCenter;
    ...
    emptySearchHistoryLabel.imageEdgeInsets = UIEdgeInsetsMake(0, 0, 0, 5);
    emptySearchHistoryLabel.autoresizingMask = UIViewAutoresizingFlexibleWidth;
    self.emptySearchHistoryLabel = (UILabel *)emptySearchHistoryLabel;
...
- (void)setupHotSearchRectangleTags
{
    ...
    // remove all subviews in hotSearFchTagsContentView
    ...
    for (int i = 0; i < self.hotSearches.count; i++) {
        ...
        rectangleTagLabel.textColor = [UIColor blackColor];
        rectangleTagLabel.backgroundColor = [UIColor clearColor];
        rectangleTagLabel.layer.borderWidth = 0.5;
        rectangleTagLabel.layer.cornerRadius = 4;
        rectangleTagLabel.layer.borderColor = PYSEARCH_COLOR(232, 232, 235).CGColor;
        rectangleTagLabel.layer.masksToBounds = YES;
        rectangleTagLabel.text = self.hotSearches[i];
        rectangleTagLabel.py_width = (contentView.py_width - 5 * PYRectangleTagMargin) / PYRectangleTagMaxCol;
        ...
        rectangleTagLabel.py_x = (rectangleTagLabel.py_width + PYRectangleTagMargin) * (i % PYRectangleTagMaxCol);
        rectangleTagLabel.py_y = (rectangleTagLabel.py_height + PYRectangleTagMargin) * (i / PYRectangleTagMaxCol)
         +PYRectangleTagMargin;
        [contentView addSubview:rectangleTagLabel];
    }
    contentView.py_height = CGRectGetMaxY(contentView.subviews.lastObject.frame ) + PYRectangleTagMargin;
